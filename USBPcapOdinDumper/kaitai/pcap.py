...
from .usbpcap import Usbpcap
from .usbmon import Usbmon
class Pcap(KaitaiStruct):
	...
	def _read(self):
		...
		while not self._io.is_eof():
			self.packets[i]=self._root.Packet(self._io, self, self._root)
			i += 1
	...
	class Packet(KaitaiStruct):
		...
		def _read(self):
			...
			try:
				...
				if _on == self._root.Linktype.usb_linux:
					self.body = Usbmon(48, io)
				elif _on == self._root.Linktype.usb_linux_mmapped:
